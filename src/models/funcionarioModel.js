...
function autenticar(email, senha) {
    var instrucaoSql = `
        SELECT idFuncionario, nome, cpf, email, senha FROM funcionario WHERE email = '${email}' AND senha = '${senha}';
    `;
...
function editar(idFuncionario, nome, cpf, email, senha) {
    ...
    var instrucaoSql = `
        UPDATE funcionario SET nome = '${nome}', cpf = '${cpf}', email = '${email}', senha = '${senha}' WHERE idFuncionario = ${idFuncionario};
    `;
    console.log("Executando a instrução SQL: \n" + instrucaoSql);
    return database.executar(instrucaoSql);
}
